...
# Config
OUTPUT_DIR = Path(__file__).parent.parent / "assets" / "tacticians"
VERSION_URL        = "https://ddragon.leagueoflegends.com/api/versions.json"
DATA_URL_TEMPLATE  = "https://ddragon.leagueoflegends.com/cdn/{version}/data/en_US/tft-tactician.json"
...
async def main():
    ...
    async with httpx.AsyncClient(timeout=HTTP_TIMEOUT) as client:
        ...
        resp.raise_for_status()
        tacticians = resp.json()["data"]

        files_to_download = [
            info["image"]["full"]
            for info in tacticians.values()
            if info["image"]["full"]
        ]
        print(f"Found {len(files_to_download)} Tacticians Image")
