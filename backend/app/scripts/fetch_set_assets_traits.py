...
# Config
TARGET_SET = 14
OUTPUT_DIR = Path(__file__).parent.parent / "assets" / "traits"
VERSION_URL        = "https://ddragon.leagueoflegends.com/api/versions.json"
DATA_URL_TEMPLATE  = "https://ddragon.leagueoflegends.com/cdn/{version}/data/en_US/tft-trait.json"
...
async def main():
    ...
    async with httpx.AsyncClient(timeout=HTTP_TIMEOUT) as client:
        ...
        resp.raise_for_status()
        traits = resp.json()["data"]

        prefix = f"Trait_Icon_{TARGET_SET}_"
        files_to_download = [
            info["image"]["full"]
            for info in traits.values()
            if info["image"]["full"].startswith(prefix)
        ]
        print(f"Found {len(files_to_download)} Set {TARGET_SET} trait")
